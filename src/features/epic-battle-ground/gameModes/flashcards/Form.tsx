...
import type { FlashcardsSettings, Operation } from '../../../solo-training-ground/types';
import { useSearchParams, useNavigate } from 'react-router-dom';
import { Lightning } from '@phosphor-icons/react';

const clamp = (v: number, min: number, max: number) => Math.max(min, Math.min(max, v));

export default function EBGFlashcardsForm() {
  ...
  const createRoom = () => {
    ...
    const roomId = Math.floor(100000 + Math.random() * 900000).toString(); // 6-digit numeric code
    const q = new URLSearchParams();
    q.set('op', settings.operation);
    q.set('mode', 'flashcards');
          ...
          <div className="bg-[#0f0f10] border border-[#2a2a2d] rounded-xl p-6 space-y-6 text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
              ...
              <input
                ...
                onChange={(e) => setSettings((s: FlashcardsSettings) => ({ ...s, numberOfDigits: clamp(parseInt(e.target.value || '0', 10), 1, 15) }))}
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
              ...
              <input
                ...
                onChange={(e) => setSettings((s: FlashcardsSettings) => ({ ...s, numberOfRows: clamp(parseInt(e.target.value || '0', 10), 1, 15) }))}
              />
            </div>
