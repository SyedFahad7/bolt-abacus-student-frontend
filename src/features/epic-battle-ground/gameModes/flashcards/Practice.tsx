...
export default function FlashcardsPractice() {
  ...
  useEffect(() => {
    ...
    if (cfg) {
      try { settings = JSON.parse(atob(cfg)); } catch (e) { /* ignore */ }
    }
  const op = searchParams.get('op') || 'addition';
  const mode = op === 'multiplication' ? 2 : op === 'division' ? 3 : 1;
  const qs = generateEpicBattleGroundQuestions({ mode, subMode: 'flash_cards', settings });
    setQuestions(qs);
  }, [searchParams]);
  ...
  return (
    <div className="p-6">
      <div className="mb-4">
        ...
        <div className="text-2xl font-bold text-gold">{q.question}</div>
      </div>
      <div className="flex gap-4">
        ...
        >Prev</button>
        <button
          className="px-4 py-2 bg-gold text-black rounded ml-auto"
          onClick={() => {
            if (index + 1 >= questions.length) {
              navigate('/student/epic-battle-ground/flashcards/results?outcome=winner');
            } else setIndex(i => i + 1);
          }}
        >Next</button>
      </div>
