...
// using local operation string union; no direct type import to avoid unused type lint
...
const EBGUntimedForm: FC = () => {
  const navigate = useNavigate();
  const [params] = useSearchParams();
  const operation = (params.get('op') as 'addition' | 'multiplication' | 'division') || 'addition';
          ...
          <div className="bg-[#0f0f10] border border-[#2a2a2d] rounded-xl p-6 space-y-6 text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, numberOfQuestions: clamp(parseInt(e.target.value) || 1, 1, 1000) })}
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, numberOfDigitsLeft: clamp(parseInt(e.target.value) || 1, 1, 15) })}
              />
            </div>
            ...
            {(operation === 'multiplication' || operation === 'division') && (
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                <label className="opacity-90">
                  {operation === 'multiplication' ? 'Number of Digits in Second Operand' : 'Number of Digits on Denominator'}
                </label>
                <input
            ...
            {operation === 'addition' && (
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <input
                  ...
                  onChange={(e) => setSettings({ ...settings, numberOfRows: clamp(parseInt(e.target.value) || 2, 1, 15) })}
                />
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
              ...
              <input
                ...
                onChange={(e) => setNumberOfPlayers(clamp(parseInt(e.target.value || '0', 10), 2, 4))}
              />
            </div>
