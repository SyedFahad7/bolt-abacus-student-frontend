...
import { Gear, Alarm } from '@phosphor-icons/react';
...
export default function SetFormPage() {
  ...
  const op = (params.get('op') as SetSettings['operation']) || 'addition';

  const [settings, setSettings] = useState<SetSettings>({
    operation: op,
  ...
  const startPractice = () => {
    if (settings.numberOfQuestions < 1 || settings.numberOfQuestions > 200) return alert('Please enter 1 to 200 questions');
    ...
    if (settings.enableTimeLimit && (settings.timeLimit < 1 || settings.timeLimit > 200)) return alert('Please enter time between 1 and 200 minutes');

    const q = new URLSearchParams();
    q.set('op', settings.operation);
    q.set('cfg', btoa(JSON.stringify(settings)));
    navigate(`/student/solo-training-ground/set/practice?${q.toString()}`);
  };
      ...
      <MainContent>
        <div className="w-full">
          <div className="flex items-center gap-2 mb-4">
            <Gear size={28} className="text-gold" weight="fill" />
            <h1 className="text-2xl font-bold text-white">Custom Challenge Settings</h1>
          </div>
          ...
          <div className="bg-[#0f0f10] border border-[#2a2a2d] rounded-xl p-6 space-y-6 text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
              ...
              <input
                ...
                type="number"
                className="md:col-span-2 bg:black bg-black border border-[#2a2a2d] rounded-lg px-3 py-2"
                value={settings.numberOfQuestions}
                onChange={(e) => setSettings((s) => ({ ...s, numberOfQuestions: clamp(parseInt(e.target.value || '0', 10), 1, 200) }))}
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
              <label className="opacity-90">
                {op === 'division' ? 'Number of Digits on Numerator' : op === 'multiplication' ? 'Number of Digits on First Operand' : 'Number of Digits'}
              </label>
              <input
            ...
            {(op === 'multiplication' || op === 'division') && (
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                <label className="opacity-90">{op === 'multiplication' ? 'Number of Digits in Second Operand' : 'Number of Digits on Denominator'}</label>
                <input
            ...
            {op === 'addition' && (
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <input
                  ...
                  onChange={(e) => setSettings((s) => ({ ...s, isZigzag: e.target.checked }))}
                />
              </div>
            )}
            ...
            {op === 'addition' && (
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <input
                  ...
                  onChange={(e) => setSettings((s) => ({ ...s, includeSubtraction: e.target.checked }))}
                />
              </div>
            )}
            ...
            {op === 'addition' && (
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <input
                  ...
                  onChange={(e) => setSettings((s) => ({ ...s, persistNumberOfDigits: e.target.checked }))}
                />
              </div>
            )}
            ...
            {op === 'division' && (
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <input
                  ...
                  onChange={(e) => setSettings((s) => ({ ...s, includeDecimals: e.target.checked }))}
                />
              </div>
            )}
            ...
            <div className="border-t border-[#2a2a2d] pt-4 mt-2 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <input
                  ...
                  onChange={(e) => setSettings((s) => ({ ...s, audioMode: e.target.checked, showQuestion: !e.target.checked }))}
                />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <select
                  ...
                  onChange={(e) => setSettings((s) => ({ ...s, audioPace: e.target.value as any }))}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <input
                  ...
                  onChange={(e) => setSettings((s) => ({ ...s, showQuestion: e.target.checked }))}
                  disabled={settings.audioMode}
                />
              </div>
            </div>
            ...
            <div className="border-t border-[#2a2a2d] pt-4 mt-2 space-y-3">
              <div className="flex items-center gap-2">
                <Alarm size={20} className="text-gold" />
                <div className="font-semibold">Optional Time Limit</div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                <label className="opacity-90">Enable</label>
                <input
                  title="enableTimeLimit"
                  type="checkbox"
                  className="md:col-span-2 h-5 w-5"
                  checked={settings.enableTimeLimit}
                  onChange={(e) => setSettings((s) => ({ ...s, enableTimeLimit: e.target.checked }))}
                />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-3">
                ...
                <input
                  title="timeLimit"
                  type="number"
                  className="md:col-span-2 bg-black border border-[#2a2a2d] rounded-lg px-3 py-2 disabled:opacity-40"
                  value={settings.timeLimit}
                  disabled={!settings.enableTimeLimit}
            ...
            <div className="pt-2">
              <button className="w-full bg-[#facb25] text-black font-semibold py-3 rounded-lg hover:bg-[#e6b422]" onClick={startPractice}>
                Start Practice
              </button>
            </div>
          </div>
