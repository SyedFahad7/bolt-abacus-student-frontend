...
export default function SetPracticePage() {
  const [params] = useSearchParams();
  const navigate = useNavigate();
  const cfg = params.get('cfg');

  const [settings, setSettings] = useState<SetSettings | null>(null);
  ...
  // Timer
  useEffect(() => {
    if (!settings || !settings.enableTimeLimit) return;
  ...
  const goToResults = (finalAnswers: QuizAnswer[], timedOut: boolean) => {
    const totalTimeMs = settings?.enableTimeLimit ? minsToMs(settings.timeLimit) - remainingMs : 0;
    const results = { questions, answers: finalAnswers, settings, timedOut, totalTimeMs };
    const encoded = btoa(JSON.stringify(results));
    navigate(`/student/solo-training-ground/set/results?results=${encoded}`);
  };
  ...
  const timeLabel = settings.enableTimeLimit ? formatDuration(remainingMs) : null;
