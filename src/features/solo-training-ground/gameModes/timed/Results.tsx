...
export default function TimedResultsPage() {
  ...
  const { questions, answers, settings, timedOut, totalTimeMs }: { questions: QuizQuestion[]; answers: QuizAnswer[]; settings: TimedSettings; timedOut: boolean; totalTimeMs: number } = JSON.parse(atob(resultsEncoded));
      ...
      <MainContent>
        <div className="w-full max-w-4xl">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <h1 className="text-3xl font-bold text-white">Time Attack - Results</h1>
          </div>
          ...
          <div className="flex gap-4">
            ...
            <button className="flex-1 bg-[#facb25] text-black font-semibold py-3 rounded-lg hover:bg-[#e6b422]" onClick={() => navigate(`/student/solo-training-ground/timed?op=${settings.operation}`)}>
              Practice Again
            </button>
          </div>
        </div>
